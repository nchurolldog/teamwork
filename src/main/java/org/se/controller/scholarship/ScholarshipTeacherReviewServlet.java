...
@WebServlet("/scholarshipTeacherReview")
public class ScholarshipTeacherReviewServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (reviewID == null || appID == null || decision == null) {
            response.sendRedirect("teacher.jsp?view=scholarshipReview&review=failed");
            return;
        }
        boolean agree = "agree".equals(decision);
        boolean success = workflowDao.completeTeacherReview(reviewID, agree, comment)
                && workflowDao.updateApplicationStatus(appID, agree ? "approved" : "teacher_rejected");
        response.sendRedirect("teacher.jsp?view=scholarshipReview&review=" + (success ? "saved" : "failed"));
    }
