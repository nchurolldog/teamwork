...
<article class="card <%= "students".equals(view) ? "span-12" : "span-8" %>">
  ...
  <table>
    <thead><tr><th>学生</th><th>班级</th><th>职位</th><th>最新成绩</th><th>出勤率</th><th>状态</th><% if ("students".equals(view)) { %><th>管理</th><% } %></tr></thead>
    <tbody>
    ...
    <tr>
      ...
      <td>-</td>
      <td><span class="pill <%= needsSupport ? "warning" : "" %>"><%= needsSupport ? "需要帮助" : "良好" %></span></td>
      <% if ("students".equals(view)) { %>
      <td>
  ...
  <div class="pagination">
    <% if (currentPage > 1) { %>
    <a class="page-link" href="teacher.jsp?view=students&q=<%= fieldValue(search) %>&classId=<%= filterClassID == null ? "" : filterClassID %>&page=<%= currentPage - 1 %>">上一页</a>
    <% } %>
    <span>第 <%= currentPage %> / <%= totalPages %> 页，共 <%= totalFilteredStudents %> 条</span>
    <% if (currentPage < totalPages) { %>
    <a class="page-link" href="teacher.jsp?view=students&q=<%= fieldValue(search) %>&classId=<%= filterClassID == null ? "" : filterClassID %>&page=<%= currentPage + 1 %>">下一页</a>
    <% } %>
  </div>
</article>
<% } %>
